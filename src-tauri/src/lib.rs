...
/// 读取 S3 配置：环境变量（CIKACHAT_S3_*）优先级最高，
/// 其次读取运行目录下的 s3-config.json，最后读取应用配置目录下的 s3-config.json。
fn load_s3_config(app: &tauri::AppHandle) -> Option<S3> {
    // 1) 环境变量
    let env = |k: &str| std::env::var(k).ok().filter(|v| !v.trim().is_empty());
    let mut cfg = s3::S3Config {
    ...
    if cfg.endpoint.is_empty() || cfg.bucket.is_empty() || cfg.access_key.is_empty() || cfg.secret_key.is_empty() {
        ...
        for dir in [std::env::current_dir().ok(), app.path().app_config_dir().ok()] {
            if let Some(d) = dir {
                let f = d.join("s3-config.json");
                if let Ok(text) = std::fs::read_to_string(&f) {
                    if let Ok(v) = serde_json::from_str::<serde_json::Value>(&text) {
                        let get = |k: &str| v.get(k).and_then(|x| x.as_str()).unwrap_or("").to_string();
                        cfg.endpoint = get("endpoint");
                        cfg.region = if get("region").is_empty() { "us-east-1".to_string() } else { get("region") };
                        cfg.bucket = get("bucket");
                        cfg.access_key = get("access_key");
                        cfg.secret_key = get("secret_key");
                        cfg.path_style = v.get("path_style").and_then(|x| x.as_bool()).unwrap_or(true);
                        cfg.public_base = get("public_base");
                        found = true;
                    }
                }
                if found {
